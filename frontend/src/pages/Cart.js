  ...
  return (
    <Container maxWidth="lg" sx={{ py: { xs: 3, md: 6 } }}>
      ...
      <Box sx={{ mb: 4 }}>
        ...
        >
          Review your items and proceed to checkout. Enjoy a seamless shopping experience!
        </Typography>
      </Box>
      <Grid container spacing={4} alignItems="flex-start">
        {/* Cart Items */}
        <Grid item xs={12} md={8}>
          <Paper elevation={0} sx={{ p: { xs: 1, md: 2 }, background: 'none', boxShadow: 'none' }}>
            <Grid container spacing={2}>
              {cart.map((item, idx) => (
                <Grid item xs={12} key={item.product._id + item.size + item.color}>
                  <Card sx={{
                    display: 'flex',
                    ...
                    <CardContent sx={{ flex: 1, p: 1 }}>
                      <Typography variant="h6" sx={{ fontWeight: 700, fontSize: { xs: '1rem', md: '1.15rem' }, mb: 0.5 }}>
                        {item.product.name}
                      </Typography>
                      ...
                      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 1 }}>
                        ...
                        <TextField
                          ...
                          inputProps={{
                            readOnly: true,
                            style: { textAlign: 'center', width: 32, fontWeight: 600 }
                          }}
                          sx={{ mx: 1, width: 40 }}
                        />
      ...
      <Dialog open={wishlistDialog} onClose={() => setWishlistDialog(false)}>
        ...
        <DialogContent>
          <Typography>Are you sure you want to move this item to your wishlist?</Typography>
        </DialogContent>
        <DialogActions>
          <Button onClick={() => setWishlistDialog(false)}
            sx={{
              py: { xs: 0.7, md: 1 },
              px: { xs: 2, md: 3 },
              fontSize: { xs: '0.92rem', md: '0.98rem' },
          ...
          >Cancel</Button>
          <Button onClick={handleWishlistConfirm} color="primary"
            sx={{
              py: { xs: 0.7, md: 1 },
              px: { xs: 2, md: 3 },
              fontSize: { xs: '0.92rem', md: '0.98rem' },
